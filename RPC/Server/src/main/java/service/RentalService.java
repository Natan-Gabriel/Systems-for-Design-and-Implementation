...
public class RentalService {
    ...
    public RentalService(Repository<Long, Rental> new_rentalRepository, MovieService movieService1, ClientService clientService1, ExecutorService executorService){
     ...
     **/
    public void addRental(Rental rental) throws ValidatorException, IOException, ClassNotFoundException, TransformerException, ParserConfigurationException, SQLException, ExecutionException, InterruptedException {
        if (!movieService.findOne(rental.getMovieID()).get().isPresent())
            throw new ValidatorException("Invalid movie ID!");
        if(!clientService.findOne(rental.getClientID()).get().isPresent())
     ...
     * @throws ParserConfigurationException if DocumentBuilderFactory or the DocumentBuilder s it creates cannot support the feature
     **/
    public void updateRental(Rental rental) throws ValidatorException, IOException, ClassNotFoundException, TransformerException, ParserConfigurationException, SQLException, ExecutionException, InterruptedException {
        if (!movieService.findOne(rental.getMovieID()).get().isPresent())
            throw new ValidatorException("Invalid movie ID!");
        if(!clientService.findOne(rental.getClientID()).get().isPresent())
            throw new ValidatorException("Invalid client ID!");
     ...
     *  from its main.java.repository(cascade deleting is called in console after deleting the given client)
     *
     ...
     *
     **/
    public Future<Set<Rental>> getAllRentals() throws IOException, ClassNotFoundException, SQLException {
        Iterable<Rental> rentals = rentalRepository.findAll();
        return executorService.submit(() -> StreamSupport.stream(rentals.spliterator(), false).collect(Collectors.toSet()));
    }
     ...
     *
     **/
    public Future<List<Long>> getMostRentedMovie() throws IOException, ClassNotFoundException, SQLException {
        Iterable<Rental> rentals = rentalRepository.findAll();
        ...
        return executorService.submit(() -> listWithoutDuplicates);
    }
    /**
     *
     ...
     *
     **/
    public Future<List<Long>> getMostActiveClient() throws IOException, ClassNotFoundException, SQLException {
        Iterable<Rental> rentals = rentalRepository.findAll();
        List<Rental> result = new ArrayList<Rental>();
        rentals.forEach(result::add);
     ...
     *
     **/
    public Future<List<Long>> getClientWithMostReturned() throws IOException, ClassNotFoundException, SQLException {
        Iterable<Rental> rentals = rentalRepository.findAll();
        List<Rental> result = new ArrayList<Rental>();
        rentals.forEach(result::add);
     ...
     *
     **/
    public Future<List<Long>> getBestClientIDFromList(List<Rental> result) throws IOException, ClassNotFoundException {
     ...
     *
     **/
    public Future<Integer> computeClientsMinusBooks() throws IOException, ClassNotFoundException, SQLException, ExecutionException, InterruptedException {
        Iterable<Rental> rentals = rentalRepository.findAll();
        List<Rental> result = new ArrayList<Rental>();
        rentals.forEach(result::add);
        ...
        Iterable<Client> clients = clientService.getAllClients().get();
        ...
        return executorService.submit(() -> listOfClients1.size()-listOfMovies1.size());
    }

    public Future<List<Rental>> getAllRentalsSorted() throws Exception {
        ...
        if (rentalRepository instanceof SortingRepository) {
            ...
            return executorService.submit(() -> StreamSupport.stream(rentals.spliterator(), false).collect(Collectors.toList()));
        } else {
